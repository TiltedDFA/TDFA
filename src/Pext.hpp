//
// Created by Malik T on 12/09/2024.
//

#ifndef TDFA_PEXT_HPP
#define TDFA_PEXT_HPP
        ...
        for (int sq = 0; sq < 64; sq++) {
            ...
            for (int type = 0; type < 2; type++) {
                ...
                for (int i = 0; i < 1 << Magics::PopCnt(mask); i++)
                    table[current_offset[type]++] = get_attacks(bb, _pdep_u64((unsigned long long)i, (unsigned long long)mask));
            }
        }
    }

    inline u64 bishop_attacks(int from, u64 occupied) {
        return bishop_table[slider_offsets[0][from] + _pext_u64((unsigned long long)occupied, (unsigned long long)slider_masks[0][from])];
    }

    inline u64 rook_attacks(int from, u64 occupied) {
        return rook_table[slider_offsets[1][from] + _pext_u64((unsigned long long)occupied, (unsigned long long)slider_masks[1][from])];
    }

    inline u64 queen_attacks(int from, u64 occupied){
