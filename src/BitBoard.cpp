...
void Position::MakeMove(Move m)
{
    ...
    if(Moves::IsPromotionMove(m)) //if not NOPROMO
    {
        ...
        pieces_[is_white][loc::PAWN] &= ~Magics::SqToBB(start_sq);
        position_key_ ^= Zobrist::PIECES[is_white][p_type][start_sq];

        if((is_white ? PiecesByColour<false>() : PiecesByColour<true>()) & target_bb)
        {
            if(target_bb & (Magics::IndexToBB<0>()  | Magics::IndexToBB<7>() | 
                            Magics::IndexToBB<56>() | Magics::IndexToBB<63>()))
            {
                //remove old caslting rights
                position_key_ ^= Zobrist::CASTLING[info_.castling_rights_];
            ...
            if(is_white)
            {
                ...
                position_key_ ^= Zobrist::PIECES[false][pt][target_sq];
            }
            else
            {
                //removing attacked piece from boards and zobrist key
                const PieceType pt = RemoveIntersectingPiece<true>(target_bb);
                position_key_ ^= Zobrist::PIECES[true][pt][target_sq];
            }
        }
        //adding the new piece that was promoted to, to the boards and zobrist key
        const PieceType promoting_to = Moves::PTypeOfProm(m);
        pieces_[is_white][promoting_to] |= target_bb;
        position_key_ ^= Zobrist::PIECES[is_white][promoting_to][target_sq];
        info_.half_moves_ = 0;
        return;
    }
    ...
    if(p_type == Moves::PAWN) 
    {
        ...
        if(target_sq == info_.en_passant_sq_)
        {
            ...
            position_key_ ^= Zobrist::EN_PASSANT[info_.en_passant_sq_];

            //resets the en passant target
            info_.en_passant_sq_ = 0; 

            // move our pawn
            pieces_[is_white][p_type] ^= Magics::SqToBB(start_sq) | target_bb; 
            position_key_ ^= Zobrist::PIECES[is_white][loc::PAWN][start_sq];
            position_key_ ^= Zobrist::PIECES[is_white][loc::PAWN][target_sq];
            ...
            if(is_white)
            {
                ...
                position_key_ ^= Zobrist::PIECES[false][loc::PAWN][target_sq - 8];
            }
            else
            {
                pieces_[true][loc::PAWN] ^= target_bb << 8;
                // RemoveIntersectingPiece<true>(target_sq + 8);
                position_key_ ^= Zobrist::PIECES[true][loc::PAWN][target_sq + 8];
            }
            return;
        }
    ...
    if(p_type == Moves::KING)
    {
        ...
        switch(EncodeKing(start_sq, target_sq))
        {
            case EncodeKing(4, 2): //queen side
                ...
                position_key_ ^= Zobrist::PIECES[true][loc::KING][4];
                //add king end sq to zobrist key
                position_key_ ^= Zobrist::PIECES[true][loc::KING][2];
                //remove old rook from zobrist key
                position_key_ ^= Zobrist::PIECES[true][loc::ROOK][0];
                // add new rook to zobrist key
                position_key_ ^= Zobrist::PIECES[true][loc::ROOK][3];
                return;
            case EncodeKing(4, 6): //king side
                ...
                position_key_ ^= Zobrist::PIECES[true][loc::KING][4];
                //add king end sq to zobrist key
                position_key_ ^= Zobrist::PIECES[true][loc::KING][6];
                //remove old rook from zobrist key
                position_key_ ^= Zobrist::PIECES[true][loc::ROOK][5];
                // add new rook to zobrist key
                position_key_ ^= Zobrist::PIECES[true][loc::ROOK][7];
                return;
            case EncodeKing(60, 58): //queen side
                ...
                //remove king start pos from zobrist key
                position_key_ ^= Zobrist::PIECES[false][loc::KING][60];
                //add king end sq to zobrist key
                position_key_ ^= Zobrist::PIECES[false][loc::KING][58];
                //remove old rook from zobrist key
                position_key_ ^= Zobrist::PIECES[false][loc::ROOK][56];
                // add new rook to zobrist key
                position_key_ ^= Zobrist::PIECES[false][loc::ROOK][59];
                return;
            case EncodeKing(60, 62): //king side
                ...
                position_key_ ^= Zobrist::PIECES[false][loc::KING][58];
                //remove old rook from zobrist key
                position_key_ ^= Zobrist::PIECES[false][loc::ROOK][63];
                // add new rook to zobrist key
                position_key_ ^= Zobrist::PIECES[false][loc::ROOK][61];
                return;
            default:
                break;
        }
    ...
    pieces_[is_white][p_type] ^= Magics::SqToBB(start_sq) | target_bb;
    position_key_ ^= Zobrist::PIECES[is_white][p_type][start_sq];
    position_key_ ^= Zobrist::PIECES[is_white][p_type][target_sq];
}
    ...
    for(U8 clr = 0; clr < 2; ++clr)
    {
        for(U8 type = 0; type < 6; ++type)
        {
            ...
            while(piece_board)
            {
                ...
                position_key_ ^= Zobrist::PIECES[clr][type][idx];
